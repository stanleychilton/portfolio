...
if(!isset($login_session)){
    ...
    $sqlcomment = "DELETE FROM comments WHERE Post_ID = $id"; 
    if(mysqli_query($conn, $sqlcomment)){
        $sql = "DELETE FROM post WHERE ID = $id"; 
        if (mysqli_query($conn, $sql)) {
            mysqli_close($conn);
            header('Location: /clickerbox/monitorcat.php'); //If book.php is your main page where you list your all records
            exit;
        } else {
