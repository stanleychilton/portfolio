...
if (isset($_POST['submit'])) {
    ...
    $cat    = ucfirst($conn->real_escape_string($_POST['cat']));
    $query   = "INSERT into categories (topic) VALUES('" . $cat . "')";
    $success = $conn->query($query);
    if (!$success) {
        die("Couldn't enter data: ".$conn->error);
    }
	header('Location: /clickerbox/adder.php');
}
?>
                ...
                <div class="col-md-6 col-md-offset-0" style = 'padding-right: 0px;padding-left: 0px;'>
					...
					if ($result->num_rows > 0) {
						...
						while($row = $result->fetch_assoc()) {
							...
							" <br><a href='category.php?id=".$row['id']."'>".$row["topic"]."</a></div>".
							"<td><a href='delete.php?id=".$row['id']."'>Delete</a></td><br><br>";
