...
if (isset($_POST['submit'])) {
    ...
    if (!$res['success']) {
		...
		$query   = "INSERT into contact (name,email,message,ip) VALUES('" . $name . "','" . $email . "','" . $message . "','" . $ip . "')";
    ...
    header('Location: /clickerbox/contactus.php');
}
            ...
            <div class="row" style="margin-right:0px;margin-left:0px">
                ...
                <div class="col-md-6 col-md-offset-0" style = 'padding-right: 0px;'>
					...
					if(isset($login_session)){
						$account = "SELECT Firstname, Lastname, Email FROM users WHERE Username = '$login_session'";
						$account_result = $conn->query($account);
						$col = $account_result->fetch_assoc();
						$name = $col['Firstname'] . " " . $col['Lastname'];
						$email = $col['Email'];
					?>						
							<form id="chardiv" action="" method="post"><br>
								Name: <input type="text" name="name" value="<?php echo $name; ?>" readonly required><br><br>
								Email: <input type="email" name="email" value="<?php echo $email; ?>" readonly required><br><br>
								messsage: <textarea class='autoExpand' rows='3' data-min-rows='3' placeholder='Put suggestion here' name="message" required></textarea><br>
