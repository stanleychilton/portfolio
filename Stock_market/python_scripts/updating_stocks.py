...
def stock(ticker):
    ...
    if response.status_code == 200:
        ...
        sql = "UPDATE current_stocks SET date = %s, time = %s, price = %s, ask = %s, range_val = %s, vol = %s, vol_avg = %s, previous_close = %s, high = %s, low = %s WHERE symbol = %s"
        stock_list = (today, time, str(price), str(ask), str(rd_high_low[0]), str(volume), str(rd_vol_avg[0].replace(',', '')), str(previous_close), str(high[0]), str(low[0]), str(ticker))
...
def main():
    print("starting an update")
    mycursor.execute("SELECT * FROM current_stocks")
