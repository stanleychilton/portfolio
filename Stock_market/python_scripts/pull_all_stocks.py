...
sql1 = "TRUNCATE TABLE all_stocks"
...
#creds = ServiceAccountCredentials.from_json_keyfile_name('stocks-slave.json', scope)
...
def stock(ticker):
    ...
    if response.status_code == 200:
        ...
        #print(stock_dic)
        sql = "INSERT INTO all_stocks (symbol, date, time, price, ask, range_val, vol, vol_avg, Previous_close, high, low) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        stock_list = (str(Stock), today, time, str(price), str(ask), str(rd_high_low[0]), str(volume), str(rd_vol_avg[0].replace(',', '')), str(previous_close), str(high[0]), str(low[0]))
